...
function initializePopup() {
  ...
  chrome.windows.getCurrent((currentWindow) => {
    // If this is the original small popup (width < 300), don't initialize
    if (currentWindow.width < 300) {
      ...
      chrome.windows.getAll({ populate: true }, (windows) => {
        const existingWindow = windows.find(w => 
          ...
          w.width >= 300  // Make sure we find the large window
        );

        if (existingWindow) {
          // Focus the existing window instead of creating a new one
          chrome.windows.update(existingWindow.id, { focused: true }, () => {
            // Close the small popup after focusing the large window
            window.close();
          });
        } else {
          ...
          chrome.windows.create({
            ...
            // Close the small popup after creating the large window
            window.close();
          });
        }
      });
